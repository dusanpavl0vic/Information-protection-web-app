...
var S0, S1, S2, S3 [256]uint64

func init() {
	copy(S0[:], Table[0:256])
	copy(S1[:], Table[256:512])
	copy(S2[:], Table[512:768])
	copy(S3[:], Table[768:1024])
}

func TigerHash(message []byte) [24]byte {
	...
	for chunkOffset := 0; chunkOffset < len(message); chunkOffset += 64 {
		...
		for i := 0; i < 4; i++ {
			for j := 0; j < 8; j++ {
				c ^= w[j]
				var cBytes [8]byte
				binary.BigEndian.PutUint64(cBytes[:], c)

				a -= S0[cBytes[0]] ^ S1[cBytes[2]] ^ S2[cBytes[4]] ^ S3[cBytes[6]]
				b += S3[cBytes[1]] ^ S2[cBytes[3]] ^ S1[cBytes[5]] ^ S3[cBytes[7]]
				b *= uint64(i + 1)
			...
			if i == 0 {
				...
				w[1] ^= w[0]
				w[2] += w[1]
				w[3] -= w[2] ^ ((w[1] ^ 0xFFFFFFFFFFFFFFFF) << 19)
				w[4] ^= w[3]
				w[5] += w[4]
				w[6] -= w[5] ^ ((w[4] ^ 0xFFFFFFFFFFFFFFFF) >> 23)
				w[7] ^= w[6]
			} else if i == 1 {
				w[0] += w[7]
				w[1] -= w[0] ^ ((w[0] ^ 0xFFFFFFFFFFFFFFFF) << 19)
				w[2] ^= w[1]
				w[3] += w[2]
				w[4] -= w[3] ^ ((w[2] ^ 0xFFFFFFFFFFFFFFFF) >> 23)
				w[5] ^= w[4]
				w[6] += w[5]
	...
	// Spoji h0, h1 i h2 u digest slice
	...
	// // Kopiraj h0, h1 i h2 u digest niz
	// copy(digest[0:8], uint64ToBytes(h0))
	// copy(digest[8:16], uint64ToBytes(h1))
	// copy(digest[16:24], uint64ToBytes(h2))

	binary.BigEndian.PutUint64(digest[0:], h0)
	binary.BigEndian.PutUint64(digest[8:], h1)
	binary.BigEndian.PutUint64(digest[16:], h2)
